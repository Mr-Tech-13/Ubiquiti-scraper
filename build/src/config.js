...
if (process.env.npm_config_conf) {
    if ((0, fs_1.existsSync)(path_1.default.resolve(__dirname, '../../' + process.env.npm_config_conf))) {
...
else if ((0, fs_1.existsSync)(path_1.default.resolve(__dirname, '../../dotenv'))) {
...
function loadProxyList(filename) {
    try {
        return (0, fs_1.readFileSync)(`${filename}.proxies`)
            .toString()
            .trim()
            .split('\n')
            .map(x => x.trim());
