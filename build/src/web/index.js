...
const webroot = (0, path_1.join)(approot, './web');
...
function sendFile(response, path, relativeTo = webroot) {
    ...
    const fsPath = (0, path_1.join)(relativeTo, path);
    const relPath = (0, path_1.relative)(relativeTo, fsPath);
    if (!relPath || relPath.startsWith('..') || (0, path_1.isAbsolute)(relPath)) {
        sendError(response, 'Bad request', 400);
        return;
    }
    try {
        const stream = (0, fs_1.createReadStream)(fsPath);
...
function handleAPI(request, response, urlComponents) {
    ...
    switch (urlComponents[1]) {
        case 'config':
            if (request.method === 'PUT') {
                ...
                request.on('end', () => {
                    ...
                    try {
                        (0, config_1.setConfig)(JSON.parse(data.join('')));
                        (0, model_1.updateStores)();
                    }
                    catch (_a) {
        ...
        case 'stores':
            ...
            return;
        case 'brands':
            sendJSON(response, (0, model_1.getAllBrands)());
            return;
        case 'series':
            sendJSON(response, (0, model_1.getAllSeries)());
            return;
        case 'models':
            sendJSON(response, (0, model_1.getAllModels)());
            return;
        case 'screenshots':
            ...
            (0, fs_1.readdir)(approot, (error, files) => {
                if (error) {
...
async function startAPIServer() {
    ...
    if (process.env.WEB_PORT) {
        server = (0, http_1.createServer)(requestListener);
