    ...
    badStatusCode(link, store, statusCode, color) {
        if (color) {
            return ('✖ ' +
                buildProductString(link, store, true) +
                ' :: ' +
                chalk_1.default.yellow(`BANNED`));
        }
        return `✖ ${buildProductString(link, store)} :: BANNED`;
    },
    ...
    maxPrice(link, store, maxPrice, color) {
        ...
        if (color) {
            return ('✖ ' +
                buildProductString(link, store, true) +
                ' :: ' +
                chalk_1.default.yellow(`IN STOCK, PRICE ${(_a = link.price) !== null && _a !== void 0 ? _a : ''} Whoa, theres a price limit ${maxPrice}`));
        }
        return `✖ ${buildProductString(link, store)} :: PRICE ${(_b = link.price) !== null && _b !== void 0 ? _b : ''} Whoa, theres a price limit ${maxPrice}`;
    },
    ...
    rateLimit(link, store, color) {
        if (color) {
            return ('✖ ' +
                buildProductString(link, store, true) +
                ' :: ' +
                chalk_1.default.yellow('TOO FAST'));
        }
