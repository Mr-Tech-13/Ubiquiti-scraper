...
async function handleProxy(request, proxy) {
    ...
    try {
        await (0, puppeteer_page_proxy_1.default)(request, proxy);
    }
    catch (error) {
...
async function lookup(browser, store) {
    var _a, _b;
    if (!(0, model_1.getStores)().has(store.name)) {
        return;
    }
    if (store.linksBuilder) {
        ...
        if (lastRunTime === -1 || Date.now() - lastRunTime > ttl) {
            ...
            try {
                await (0, fetch_links_1.fetchLinks)(store, browser);
    ...
    for (const link of store.links) {
        if (!(0, filter_1.filterStoreLink)(link)) {
            continue;
        }
        ...
        await page.setUserAgent(await (0, util_1.getRandomUserAgent)());
        ...
        if (useAdBlock) {
            ...
            await (0, adblocker_1.enableBlockerInPage)(pageProxy);
        }
        await page.setRequestInterception(true);
        ...
        if (pageProxy) {
            await (0, adblocker_1.disableBlockerInPage)(pageProxy);
        }
        ...
        await (0, backoff_1.processBackoffDelay)(store, link, statusCode);
        await (0, util_1.closePage)(page);
        if (customContext) {
            await context.close();
        }
    }
...
async function lookupIem(browser, store, page, link) {
    ...
    if (!(0, util_1.isStatusCodeInRange)(statusCode, successStatusCodes)) {
        return statusCode;
    }
    if (await isItemInStock(store, page, link)) {
        ...
        if (config_1.config.browser.open) {
            await (link.openCartAction === undefined
                ? (0, open_1.default)(givenUrl)
                : link.openCartAction(browser));
        }
        (0, messaging_1.sendNotification)(link, store);
...
async function checkIsCloudflare(store, page, link) {
    ...
    if (await (0, includes_labels_1.pageIncludesLabels)(page, cloudflareLabel, baseOptions)) {
...
async function isItemInStock(store, page, link) {
    ...
    if (store.labels.captcha) {
        if (await (0, includes_labels_1.pageIncludesLabels)(page, store.labels.captcha, baseOptions)) {
            ...
            if (config_1.config.captchaHandler.service && store.labels.captchaHandler) {
                ...
                if (!(await (0, captcha_handler_1.handleCaptchaAsync)(page, store))) {
                ...
                await (0, util_1.delay)((0, util_1.getSleepTime)(store));
                return false;
            }
        }
    }
    if (store.labels.bannedSeller) {
        if (await (0, includes_labels_1.pageIncludesLabels)(page, store.labels.bannedSeller, baseOptions)) {
            logger_1.logger.warn(logger_1.Print.bannedSeller(link, store, true));
            return false;
        }
    }
    if (store.labels.outOfStock) {
        if (await (0, includes_labels_1.pageIncludesLabels)(page, store.labels.outOfStock, baseOptions)) {
    ...
    if ((_b = link.labels) === null || _b === void 0 ? void 0 : _b.inStock) {
        ...
        if (!(await (0, includes_labels_1.pageIncludesLabels)(page, link.labels.inStock, options))) {
            logger_1.logger.info(logger_1.Print.outOfStock(link, store, true));
            return false;
        }
    }
    if (store.labels.inStock) {
        ...
        if (!(await (0, includes_labels_1.pageIncludesLabels)(page, store.labels.inStock, options))) {
            logger_1.logger.info(logger_1.Print.outOfStock(link, store, true));
            return false;
        }
    }
...
async function tryLookupAndLoop(browser, store) {
    ...
    const sleepTime = (0, util_1.getSleepTime)(store);
